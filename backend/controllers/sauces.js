...
exports.getAllSauce = (req, res, next) => {
  Sauce.find()
    ...
    .catch((error) =>
      res.status(404).jsons({ error: "Oups! Ça n'a pas marché." })
    );
};
...
exports.postSauce = (req, res, next) => {
  ...
  sauce
    ...
    .catch((error) =>
      res.status(400).json({ error: "Oups! Ça n'a pas marché." })
    ); // ajouter msg d'erreur
};
...
exports.deleteSauce = (req, res, next) => {
  Sauce.findOne({ _id: req.params.id })
    .then((sauce) => {
      if (sauce.userId === req.userIdToken) { // à reporter sur modifier une sauce
        ...
        fs.unlink(`images/${filename}`, () => { // suppression du fichier image
		  ...
		  res.status(403).json({ error: 'pas touche!'});
	  }
    })
    .catch((error) =>
      res.status(400).json({ error: "Oups! Ça n'a pas marché." })
